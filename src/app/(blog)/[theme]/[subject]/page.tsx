...
export default async function Subject({ params }: { params: Promise<Params> }) {
  ...
  const themeResult = posts.find(post => post.theme === theme);
  const postsTheme = themeResult?.content.find(subject => subject.subject === subjectParams);

  return (
    <main className="w-10/12 mx-auto mt-6 max-h-screen">
      <h2 className="text-lg tracking-wider">{subjectParams}</h2>
          ...
          <CarouselContent className="flex gap-2 ml-1">
            {
              postsTheme?.content.map(post => {
                return (
                  <Link key={post.title} href={`/${theme}/${subjectParams}/${post.title}`} className="basis-1/4">
